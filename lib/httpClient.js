...
function Client (opts) {
  ...
  this.parser[HTTPParser.kOnMessageComplete] = () => {
    ...
    if (!this.destroyed) {
      let options = clone(this.opts);
      let body = clone(resp.body);
      
      try {

        const isFn = typeof options.verifyBody === 'function'
        if (isFn && !options.verifyBody(body)) {
          this.emit('mismatch', body)
        } else if (!isFn && options.expectBody && options.expectBody !== body) {
          this.emit('mismatch', body)
        }
      } catch (error) {
        console.log(error);
